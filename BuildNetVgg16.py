...
#-----------------------------Build network encoder based on VGG16 network and load the trained VGG16 weights-----------------------------------------
       #Layer 1
        ...
        self.RelevanceMap = tf.nn.bias_add(tf.nn.conv2d(tf.cast(ROIMap,tf.float32), W, [1, 1, 1, 1], padding="SAME"),B) # apply covolution add bias

        self.conv1_2 = self.conv_layer(tf.nn.relu(self.conv1_1*self.RelevanceMap), "conv1_2")#Build Convolution layer +Relu and load weights
